...
class Smartaddons_Scroller_Block_List extends Mage_Core_Block_Template
{
	...
	public function __construct($attributes = array()){
		parent::__construct();
		$this->_config = Mage::helper('scroller/data')->get($attributes);
	}

	public function getConfig($name=null, $value=null){
		if (is_null($this->_config)){
			$this->_config = Mage::helper('scroller/data')->get(null);
		}
	...
	protected function _toHtml(){
		...
		//$template_file = "smartaddons/scroller/block_template_default.phtml";
		$template_file = "smartaddons/scroller/". $this->_config['theme'].".phtml";
	...
	protected function _getMedia(){
		$items = Mage::getStoreConfig('scroller_cfg/product_selection/product_additem');	//kq:  string(85) "a:1:{s:18:"_1289468104191_191";a:2:{s:6:"regexp";s:4:"6:20";s:5:"value";s:4:"8:20";}}" 
		if($items){
	...
	protected function _getItems(){
		...
		foreach( $products as $k => $_product ) {
			...
			if($_product->getImage()){
				...
				$item['thumb'] = $this->getSkinUrl('smartaddons/scroller/images/no_image.png');
			}		
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('scroller_cfg/advanced/include_jquery')){
				// if module allowed jquery.
				$import_str .= $jsHelper->includeSkinScript('smartaddons/scroller/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('smartaddons/scroller/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.scroller.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('smartaddons/scroller/js/script.js');
			
			Mage::register('jsmart.scroller.js', 1);
		}
		return $import_str;
	}
	...
	private function _getResizedImage($Obj, $width, $height, $quality = 100) {
		...
		if(file_exists($imageResized)){
			...
			return $this->getSkinUrl('smartaddons/scroller/images/no_image.gif');
		}
	}		
}
